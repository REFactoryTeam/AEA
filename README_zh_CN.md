...
* **高级终端：** 拥有两种模式的多功能终端：
  * **合成模式：**
    * **模糊替换：** 从网络补全原料时支持物品的模糊匹配。
    * **流体填充：** 自动从网络中充满消耗后的流体容器（如水桶）。
    * **自动回收：** 合成返还物（如空桶、玻璃瓶）会自动返回 AE 网络，避免干扰连续合成。
  * **处理模式：** 方便地从网络中一键提取配方所需的各类原料。
  * **智能记忆网格（共有特性）：** 转移配方时会自动记录缺失物品；一旦缺失物品补齐，它们会自动进入网格，无需再次点击 JEI 转移配方。
